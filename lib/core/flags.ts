...
// UI flag: show any coach price to athletes — hourly rate, per-service price
// (card and profile), the price sort options and the price range filter on
// /coaches. Off while the commercial model is based on club packages/
// subscriptions (see the "Pacchetti" section on the landing). The price data
// and rendering code are kept intact — flip this to `true` to re-enable
// everywhere.
